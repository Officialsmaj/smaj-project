...
function createMemberCard(member, index) {
    ...
    if (member.skills?.length) {
        ...
        member.skills.slice(0, 3).forEach(value => { const tag = document.createElement("span"); tag.textContent = value; skills.append(tag); });
        card.append(skills);
    }
...
function createSocialLinks(member, primaryOnly = false) {
    ...
    Object.entries(member.social_links || {}).forEach(([network, url]) => {
        if (primaryOnly && network !== "linkedin") return;
